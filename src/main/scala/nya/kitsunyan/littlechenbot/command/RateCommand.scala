...
trait RateCommand extends Command with ExtractImage {
  ...
  private def handleMessageInternal(implicit message: Message, arguments: Arguments, locale: Locale): Future[Status] = {
    ...
    def replyWithRating(everypixelData: EverypixelData): Future[Message] = {
      ...
      val messageFuture = request(SendSticker(message.source, InputFile(ratings(quality)),
      ...
      if (everypixelData.tags.nonEmpty) {
        messageFuture.flatMap { message =>
          ...
          request(SendMessage(message.source, text, replyToMessageId = Some(message.messageId)))
        }
      } else {
        messageFuture
      }
